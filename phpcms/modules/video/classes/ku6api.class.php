...
class ku6api {
	...
	public function check($vid = '') {
		...
		$postdata['vid'] = $vid;
		$postdata['url'] = APP_PATH . 'api.admin?op=video_api';
		//向vms post数据，并获取返回值
		if ($data = $this->post($postdata)) {
			return true;
		} else {
	...
	public function update_vms_member($data = array()) {
		...
		$api_url = pc_base::load_config('sub_config','member_add_dir').'MemberUpgrade.admin';
	...
	*/
	...
	public function complete_info($data){
		//构造post数据
		$postdata = $data; 
		$postdata['user_back'] = APP_PATH . 'api.admin?op=video_api';
 		//向vms post数据，并获取返回值 
		
		$url = $this->ku6api_api."CompleteInfo.admin";
	...
	*/
	public function Get_Complete_Info($data){
		if (empty($data)) return false; 
		$url = $this->ku6api_api."Get_Complete_Info.admin";
	...
	*/
	public function check_user_back($url){
		...
		$data['url'] = $url;
		$url = $this->ku6api_api."Check_User_Back.admin";
		$return_data = $this->post_api($url, $data);
   		if ($return_data['code']=='200') { 
   			return 200;
		} else { 
	...
	public function send_code($data){
		...
		$url = $this->ku6api_api."Send_Code.admin";
	...
	public function check_email_code($data){
		if (empty($data)) return false;  
		$url =  $this->ku6api_api."Check_Email_Code.admin";
		$return_data = $this->post_api($url, $data); 
		if($return_data['code']=='200'){
			return $return_data['data'];
		}else{
