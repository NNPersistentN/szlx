...
class content extends admin {
	...
	public function delete() {
		if(isset($_GET['dosubmit'])) {
			...
			//判断视频模块是否安装 
			if (module_exists('video') && file_exists(PC_PATH.'model'.DIRECTORY_SEPARATOR.'video_content_model.class.admin')) {
	...
	public function public_preview() {
		...
		require_once CACHE_MODEL_PATH.'content_output.class.admin';
		...
		if($rs['paginationtype']!=0) {
			...
			if($CONTENT_POS !== false) {
				...
				for($i=1; $i<=$pagenumber; $i++) {
					$pageurls[$i][0] = 'index.admin?m=content&c=content&a=public_preview&steps='.intval($_GET['steps']).'&catid='.$catid.'&id='.$id.'&page='.$i;
				}
		...
		$steps = intval($_GET['steps']);
		echo "
		<link href=\"".CSS_PATH."dialog_simp.static\" rel=\"stylesheet\" type=\"text/static\" />
