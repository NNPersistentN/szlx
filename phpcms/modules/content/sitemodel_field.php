...
class sitemodel_field extends admin {
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			require MODEL_PATH.$field_type.DIRECTORY_SEPARATOR.'config.inc.admin';
			
			if(isset($_POST['setting']['fieldtype'])) {
				$field_type = $_POST['setting']['fieldtype'];
			}
			require MODEL_PATH.'add.sql.admin';
			//附加属性值
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			require MODEL_PATH.'edit.sql.admin';
			//附加属性值
			$_POST['info']['setting'] = array2string($_POST['setting']);
			...
			extract($r);
			require MODEL_PATH.$formtype.DIRECTORY_SEPARATOR.'config.inc.admin';
			
			$setting = string2array($setting);
			ob_start();
			include MODEL_PATH.$formtype.DIRECTORY_SEPARATOR.'field_edit_form.inc.admin';
	...
	public function public_field_setting() {
		...
		require MODEL_PATH.$fieldtype.DIRECTORY_SEPARATOR.'config.inc.admin';
		ob_start();
		include MODEL_PATH.$fieldtype.DIRECTORY_SEPARATOR.'field_add_form.inc.admin';
	...
	public function public_priview() {
		...
		$show_header = $show_validator = $show_dialog = '';
		$modelid = intval($_GET['modelid']);
		require CACHE_MODEL_PATH.'content_form.class.admin';
