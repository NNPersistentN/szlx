...
class album extends admin {
	...
	public function __construct() {
		...
		if (!module_exists('video')) {
			showmessage(L('please_setting_video_info'), 'index.admin?m=admin&c=module&a=init');
		}
		if (!$this->setting) showmessage(L('please_not_setting_info'), 'index.admin?m=video&c=video&a=setting');
	...
	public function import() {
		...
		if ($id) {
			...
			if ($result) showmessage(L('album_add_success'), 'index.admin?m=special&c=special');
