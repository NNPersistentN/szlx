...
class attachments {
	...
	public function crop_upload() {
		if (isset($GLOBALS["HTTP_RAW_POST_DATA"])) {
			...
			if (isset($_GET['file']) && !empty($_GET['file'])) {
				$_GET['file'] = str_ireplace(array(';','admin'),'',$_GET['file']);
				if(is_image($_GET['file'])== false || stripos($_GET['file'],'.admin')!==false) exit();
	...
	final public static function admin_tpl($file, $m = '') {
		...
		return PC_PATH.'modules'.DIRECTORY_SEPARATOR.$m.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$file.'.tpl.admin';
	}
