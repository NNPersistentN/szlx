...
class file extends admin {
	...
	public function __construct() {
		...
		if (file_exists($this->filepath.'config.admin')) {
			$this->style_info = include $this->filepath.'config.admin';
	...
	public function updatefilename() {
		...
		@file_put_contents($this->filepath.'config.admin', '<?admin return '.var_export($this->style_info, true).';?>');
	...
	public function edit_file() {
		...
		if ($_POST['dosubmit']) {
			...
			$code = str_replace(array('<?','{admin'),array('<？','{ admin'),$code);
			if ($is_write == 1) {
	...
	public function edit_pc_tag() {
		...
		switch ($op) {
			...
			default:
				if (!file_exists(PC_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.$op.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$op.'_tag.class.admin')) {
