...
class style extends admin {
	...
	public function disable() {
		...
		$filepath = $this->filepath.$style.DIRECTORY_SEPARATOR.'config.admin';
		if (file_exists($filepath)) {
			...
			if (is_writable($filepath)) {
				file_put_contents($filepath, '<?admin return '.var_export($arr, true).';?>');
			} else {
	...
	public function import() {
		if (isset($_POST['dosubmit'])) {
			...
			//echo $this->filepath.$code['dirname'].DIRECTORY_SEPARATOR.'config.admin';
			if (!file_exists($this->filepath.$code['dirname'].DIRECTORY_SEPARATOR.'config.admin')) {
				...
				if (@is_writable($this->filepath.$code['dirname'].DIRECTORY_SEPARATOR)) {
					@file_put_contents($this->filepath.$code['dirname'].DIRECTORY_SEPARATOR.'config.admin', '<?admin return '.var_export($code, true).';?>');
	...
	public function updatename() {
		...
		if (is_array($name)) {
			foreach ($name as $key=>$val) {
				$filepath = $this->filepath.$key.DIRECTORY_SEPARATOR.'config.admin';
				if (file_exists($filepath)) {
					$arr = include $filepath;
					$arr['name'] = $val;
				} else {
