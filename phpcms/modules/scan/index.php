...
class index extends admin {
	...
	protected $safe = array ('file_type' => 'admin|js','code' => '','func' => 'com|system|exec|eval|escapeshell|cmd|passthru|base64_decode|gzuncompress','dir' => '', 'md5_file'=>'');
	...
	public function init() {
		...
		if (file_exists(CACHE_PATH.'caches_scan'.DIRECTORY_SEPARATOR.'caches_data')) {
			$md5_file_list = glob(CACHE_PATH.'caches_scan'.DIRECTORY_SEPARATOR.'caches_data'.DIRECTORY_SEPARATOR.'md5_*.admin');
	...
	public function public_file_func() {
		...
		if (isset($scan['func']) && !empty($scan['func'])) {
			foreach ($file_list as $key=>$val) {
				...
				if(stristr($key,'.admin.') != false || preg_match_all('/[^a-z]?('.$scan['func'].')\s*\(/i', $html, $state, PREG_SET_ORDER)) {
	...
	public function public_file_code() {
		...
		if (isset($scan['code']) && !empty($scan['code'])) {
			foreach ($file_list as $key=>$val) {
				$html = file_get_contents(PHPCMS_PATH.$key);
				if(stristr($key, '.admin.') != false || preg_match_all('/[^a-z]?('.$scan['code'].')/i', $html, $state, PREG_SET_ORDER)) {
					$badfiles[$key]['code'] = $state;
	            }
	            if(strtolower(substr($key, -4)) == '.admin' && function_exists('zend_loader_file_encoded') && zend_loader_file_encoded(PHPCMS_PATH.$key)) {
	...
	public function view() {
		...
		//判断文件名，如果是database.admin 对里面的关键字符进行替换
		$basename = basename($url);
		if($basename == "database.admin" || $basename == "system.admin"){
			//$html = str_replace();
	...
	public function md5_creat() {
		...
		switch ($pro) {
			case '1'://统计文件
				...
				ob_flush();
				ob_clean();
				$list = scan_file_lists(PHPCMS_PATH, 1, 'admin', 0, 1);
