...
class plugin extends admin {
	...
	public function init() {
		...
		if($pluginfo = $this->db->select('','*','','disable DESC,listorder DESC')) {
			foreach ($pluginfo as $_k=>$_r) {
				if(file_exists(PC_PATH.'plugin'.DIRECTORY_SEPARATOR.$_r['dir'].DIRECTORY_SEPARATOR.$_r['dir'].'.class.admin')){
					$pluginfo[$_k]['url'] = 'plugin.admin?id='.$_r['dir'];
				} else {
					$pluginfo[$_k]['url'] = '';
				}
		...
		if(!isset($_GET['dir'])) {
			...
			while (false !== ($entry = $pluginsdir->read())) {
				...
				if(!in_array($entry, array('.', '..')) && is_dir(PC_PATH.'plugin'.DIRECTORY_SEPARATOR.$entry) && !in_array($entry, $installsdir) && !$this->db->get_one(array('identification'=>$entry))) {
					$config_file = PC_PATH.'plugin'.DIRECTORY_SEPARATOR.$entry.DIRECTORY_SEPARATOR.'plugin_'.$entry.'.cfg.admin';
			...
			$dir = trim($_GET['dir']);
			$license = 0;
			$config_file = PC_PATH.'plugin'.DIRECTORY_SEPARATOR.$dir.DIRECTORY_SEPARATOR.'plugin_'.$dir.'.cfg.admin';
			if(file_exists($config_file)) {
				$plugin_data = @require($config_file);
	...
	public function design() {
	    ...
	    if(isset($_POST['dosubmit'])) {
			...
			$data['plugin'] = array(
							...
							'description' => "",
							'installfile' => 'install.admin',
							'uninstallfile' => 'uninstall.admin',
						);

			
			$filepath = PC_PATH.'plugin'.DIRECTORY_SEPARATOR.$data['identification'].DIRECTORY_SEPARATOR.'plugin_'.$data['identification'].'.cfg.admin';
			pc_base::load_sys_func('dir');
	...
	public function appcenter() {
		...
		$data = file_get_contents('http://open.phpcms.cn/index.admin?m=open&c=api&a=get_applist&s='.$s.'&p='.$p);
		$data = array_iconv(json_decode($data, true),'utf-8',CHARSET);
		
		$recommed_data = file_get_contents('http://open.phpcms.cn/index.admin?m=open&c=api&a=get_recommed_applist&s=5&p=1');
		$recommed_data = array_iconv(json_decode($recommed_data, true),'utf-8',CHARSET);
		
		$focus_data = file_get_contents('http://open.phpcms.cn/index.admin?m=open&c=api&a=get_app_focus&num=3');
	...
	public function appcenter_detail() {
		$data = array();
		$id = intval($_GET['id']);
		$data = file_get_contents('http://open.phpcms.cn/index.admin?m=open&c=api&a=get_detail_byappid&id='.$id);
		$data = array_iconv(json_decode($data, true),'utf-8',CHARSET);
		extract($data);		
		if($appname) {
...
?>
EOF;
					$uninstallres = @file_put_contents($appdirname.DIRECTORY_SEPARATOR.'uninstall.admin', $installdata);
					$installres = @file_put_contents($appdirname.DIRECTORY_SEPARATOR.'install.admin', $installdata);
...
?>				
EOF;
					$cfgres = @file_put_contents($appdirname.DIRECTORY_SEPARATOR.'plugin_'.$data['appenname'].'.cfg.admin', $cfgdata);
					
					//检查配置文件是否写入成功
					if($installres*$uninstallres*$cfgres > 0) {
						showmessage(L('plugin_configure_success', '', 'plugin'), 'index.admin?m=admin&c=plugin&a=import&dir='.$data['appenname']);
					} else {
			...
			if(empty($upgradezip_url)) {
				showmessage(L('download_fail', '', 'plugin'), 'index.admin?m=admin&c=plugin&a=appcenter');
			}
			...
			if($this->copyfailnum > 0) {
				...
				showmessage(L('download_success', '', 'plugin'), 'index.admin?m=admin&c=plugin&a=import&dir='.$appdirname);
			}
		}
	}
	...
	public function config() {
		if(isset($_POST['dosubmit'])) {
			...
			if(!isset($_GET['module'])) {	
				...
				$plugin_admin_path = PC_PATH.'plugin'.DIRECTORY_SEPARATOR.$identification.DIRECTORY_SEPARATOR.'plugin_admin.class.admin';
	...
	function set_hook_cache() {
		if($info = $this->db->select(array('disable'=>1),'*','','listorder DESC')) {
			foreach($info as $i) {
				$id = $i['identification'];
				$hook_file = PC_PATH.'plugin'.DIRECTORY_SEPARATOR.$id.DIRECTORY_SEPARATOR.'hook.class.admin';
