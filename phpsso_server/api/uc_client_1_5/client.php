...
/*
	...
	$Id: client.admin 963 2009-09-21 02:40:04Z zhaoxiongfei $
*/
...
function uc_api_post($module, $action, $arg = array()) {
	...
	return uc_fopen2(UC_API.'/index.admin', 500000, $postdata, '', TRUE, UC_IP, 20);
}
...
function uc_api_url($module, $action, $arg='', $extra='') {
	$url = UC_API.'/index.admin?'.uc_api_requestdata($module, $action, $arg, $extra);
	return $url;
}
...
function uc_api_mysql($model, $action, $args=array()) {
	...
	if(empty($uc_controls[$model])) {
		include_once UC_ROOT.'./lib/db.class.admin';
		include_once UC_ROOT.'./model/base.admin';
		include_once UC_ROOT."./control/$model.admin";
...
function uc_serialize($arr, $htmlon = 0) {
	include_once UC_ROOT.'./lib/xml.class.admin';
...
function uc_check_avatar($uid, $size = 'middle', $type = 'virtual') {
	$url = UC_API."/avatar.admin?uid=$uid&size=$size&type=$type&check_file_exists=1";
